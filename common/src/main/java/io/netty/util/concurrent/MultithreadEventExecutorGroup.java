...
public abstract class MultithreadEventExecutorGroup extends AbstractEventExecutorGroup {
     ...
     * @param executorServiceFactory    the {@link ExecutorServiceFactory} to use, or {@cod
     * e null} if the default
     *                                  should be used.
    ...
    private MultithreadEventExecutorGroup(int nEventExecutors,
        ...
        if (executor == null) {
            executor = newDefaultExecutorService(nEventExecutors);  //cores=4 *2 = 8
            shutdownExecutor = true;
        }
        //System.out.println("nEventExecutors :" + nEventExecutors) ;
        children = new EventExecutor[nEventExecutors];
        if (isPowerOfTwo(children.length)) { // cores=4 *2 = 8  true
